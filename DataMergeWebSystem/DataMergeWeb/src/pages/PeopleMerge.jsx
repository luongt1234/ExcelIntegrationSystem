...
export default function PeopleMerge() {
  ...
  const EXCLUDED_KEYWORDS = ['chức vụ', 'phòng ban', 'phòng', 'ban', 'đơn vị', 'câu hỏi', 'gk1', 'gk2', 'gk3', 'gk4', 'gk5', 'điểm', 'thời gian', 'ghi chú', 'kết quả', 'stt', 'chuẩn bị', 'điều kiện', 'col', 'đánh giá', 'nhận xét'];
        ...
        {step === 0 && (
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setStep(1)} className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors flex items-center space-x-1 shadow-sm">
                <span>Tiếp theo →</span>
              </button>
            </div>
            {MODES.map(m => (
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => setStep(0)} className="px-3.5 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors">← Quay lại</button>
                <button onClick={() => setStep(2)} disabled={uploadedFiles.filter(Boolean).length === 0} className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1 shadow-sm">
                  <span>Tiếp theo →</span>
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
              ...
              <div className="flex items-center space-x-3">
                {verifiedFiles.length === uploadedFiles.filter(Boolean).length ? (
                  <span className="text-xs text-green-600 font-medium">✅ Tất cả {uploadedFiles.filter(Boolean).length} file đã xác nhận</span>
                ) : (
                  <span className="text-xs text-orange-500 font-medium">⚠️ Đang chờ xác nhận ({verifiedFiles.length}/{uploadedFiles.filter(Boolean).length})</span>
                )}
                <button onClick={() => setStep(1)} className="px-3.5 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors">← Quay lại upload</button>
                <button 
                  onClick={() => setStep(3)} 
                  disabled={verifiedFiles.length !== uploadedFiles.filter(Boolean).length || uploadedFiles.filter(Boolean).length === 0} 
                  className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1 shadow-sm"
                >
                  <span>Tiếp theo →</span>
                </button>
              </div>
            ...
            <div className={`grid grid-cols-1 ${uploadedFiles.filter(Boolean).length > 1 ? 'lg:grid-cols-2' : ''} gap-4`}>
              {uploadedFiles.filter(Boolean).map((file) => (
                <FileStructurePreview 
                  ...
                  savedMapping={fileMappings[file.fileId]}
                  onSheetChange={(sheetName) => handleSheetChange(file.fileId, sheetName)}
                  previewState={previewStates?.[file.fileId]}
                  onPreviewStateChange={(newState) => setPreviewStates(prev => ({ ...prev, [file.fileId]: newState }))}
                />
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs text-gray-500">Các file đã xác nhận cấu trúc. Chọn khóa kết hợp để phát hiện bản ghi trùng.</p>
              </div>
              <div className="flex items-center space-x-2">
                <button onClick={() => setStep(2)} className="px-3.5 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors">← Quay lại kiểm tra</button>
                <button onClick={handleMerge} disabled={loading} className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-1 shadow-sm">
        ...
        {step === 4 && result && (
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={() => setStep(3)} className="px-3.5 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors">← Quay lại Chọn Khóa</button>
                <button onClick={() => { resetPeopleState(); setMode(1); }} className="px-3.5 py-1.5 border border-gray-300 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-50 transition-colors">🔄 Bắt đầu lại</button>
                <button onClick={handleExport} className="px-4 py-1.5 bg-green-600 text-white rounded-lg text-xs font-medium hover:bg-green-700 transition-colors shadow-sm">📥 Xuất Excel</button>
              </div>
            </div>
